...
async def search(message: types.Message, state=FSMContext):
    ...
    # if message.text != "🎲 Tasodifiy kitob":    
        ...
        if status_code == 200:
            ...
            msg = f"<b>❌ TOPILMADI !!!</b>\n\nQidirilgan kitob: {message.text}\nQidirgan foydalanuvchi: {message.from_user.full_name} (<a href='tg://user?id={message.from_user.id}'>{('@'+ message.from_user.username) if message.from_user.username else (message.from_user.first_name)}</a>) || ID: <code>{message.from_user.id}</code>"
            await bot.send_message(chat_id=ADMINS[0], text=msg)
            ...
            if obj['file_link'] == book_link:
                ...
                try:
                    if user_id in ADMINS:
                        await callback_query.message.answer(text = f"KITOB LINKI: <code>{book_link}</code>")
                    else:   
